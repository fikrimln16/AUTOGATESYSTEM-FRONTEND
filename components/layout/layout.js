  ...
  useEffect(() => {
    const getData = async () => {
      const res = await Promise.all([
        axios.get(`https://web-production-1a2c.up.railway.app/security/withmask/jumlah/${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`),
        axios.get(`https://web-production-1a2c.up.railway.app/security/nomask/jumlah/${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`),
      ]);
  ...
  return (
    <div className=" font-[Helvetica]">
      ...
      <div className="relative md:ml-64 ">
        ...
        {/* Header */}
        {role === "security" ? <HeaderStats jumlahMask={jumlah} jumlahNoMask={jumlahNoMask} role={role} /> : <HeaderStats role={role} allUser={allUser} jumlahMonth={jumlahMonth} cardMonth={cardMonth} />}
