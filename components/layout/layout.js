    ...
    useEffect(() => {
      const getData = async () => {
        const res = await Promise.all([
          axios.get(`https://web-production-7522.up.railway.app/security/withmask/jumlah/${date.getFullYear()}-${date.getMonth()+1}-${date.getDate()}`),
          axios.get(`https://web-production-7522.up.railway.app/security/nomask/jumlah/${date.getFullYear()}-${date.getMonth()+1}-${date.getDate()}`)
        ])
